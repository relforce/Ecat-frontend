...
import { BACKEND_URL, MULTI_TAPS } from "../../constants";
...
const MultiTap = () => {
  ...
  const handleClick = async () => {
    if (point < MULTI_TAPS[multiTapLevel + 1].point) {
      toast.error("Your point is not enough");
      return false;
    }

    try {
      const response = await axios.post(`${BACKEND_URL}/api/user/multi-tap`);
      const { data } = response;
      if (data?.user) {
        setUser(data?.user);
        setPointPerClick(data?.user?.pointPerClick);
        setMultiTapLevel(data?.user?.multiTapLevel);
