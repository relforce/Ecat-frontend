...
import { BACKEND_URL, YOUTUBE_CHANNEL } from "../../constants";
import axios from "axios";
import { PointContext } from "../../state/PointContext";
import { Link } from "react-router-dom";
import toast from "react-hot-toast";

const YoutubeChannelDialog = ({ setDialogOpen, animateLeaves }) => {
  ...
  const handleClick = async () => {
    if (!joined) {
      toast.error("You haven't joined yet. Please join now!");
      return false;
    }

    try {
      const response = await axios.post(
        `${BACKEND_URL}/api/task/channel-task`,
        {
      ...
      console.log("app@Multi-Tap", e.message);
    } finally {
      setDialogOpen(false);
      return false;
    }
  ...
  return (
    <div className="bottom-sheet-scroll">
      <div className="bs-content">
        ...
        <div className="bs-content-title">Join our Youtube channel</div>
        <div className="bs-content-description">
          These are 6 cryptography experts people thought could Satoshi Nakamoto
        </div>
        <div className="bs-content-description">
          <Link
            to={YOUTUBE_CHANNEL}
...
export default YoutubeChannelDialog;
