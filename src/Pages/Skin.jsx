...
import { BACKEND_URL, LEVEL_DATA, SKIN_DATA, SKINS } from "../constants";
import { PointContext } from "../state/PointContext";
import axios from "axios";
import { Link, useNavigate } from "react-router-dom";

function Skin() {
  const { skin, setSkin, setUser, level, point, user } =
    useContext(PointContext);
  const [currentSkin, setCurrentSkin] = useState(skin);

  const navigate = useNavigate();

  const handleSkinPurchase = async (_skin) => {
    try {
      const response = await axios.post(`${BACKEND_URL}/api/user/buy-skin`, {
        name: _skin,
      });
      ...
      <div className="page">
        <main className="main">
          <div className="top-bar">
            <Link to="/" className="top-bar-left">
              <div className="icon is-flipped is-24">
                <svg
          ...
          <div className="profile">
            ...
            <div className="profile-skins">
              ...
              <div className="profile-skin">
                <div className="profile-skin-image">
                  <div
                    className="profile-skin-prev"
                    onClick={() => {
                      setCurrentSkin((prev) =>
                        SKINS.indexOf(prev) > 0
                          ? SKINS[SKINS.indexOf(prev) - 1]
                          : "Default"
                      );
                    }}
                  >
                    <div className="icon is-flipped">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        xmlSpace="preserve"
                  ...
                  <div
                    className="profile-skin-next"
                    onClick={() => {
                      setCurrentSkin((prev) =>
                        SKINS.indexOf(prev) < SKINS.length - 1
                          ? SKINS[SKINS.indexOf(prev) + 1]
                          : "Default"
                      );
                    }}
                  ...
                  <picture>
                    <source
                      srcSet={
                        currentSkin == SKINS[0]
                          ? `/images/hamsters/${level}.png`
                          : SKIN_DATA[currentSkin].url
                      }
                      type="image/webp"
                    />
                    <img
                      className="img-responsive block mx-auto max-h-[100%]"
                      src={
                        currentSkin == SKINS[0]
                          ? `/images/hamsters/${level}.png`
                          : SKIN_DATA[currentSkin].url
                ...
                <div className="profile-skin-info">
                  <div className="profile-skin-title">{currentSkin}</div>
                  <div className="profile-skin-description">
                    {SKIN_DATA[currentSkin].description}
                  </div>
                  <div className="profile-skin-status">
                    {skin == currentSkin ? (
                      <div className="is-purchased">Purchased</div>
                    ) : (
                      <div className="price is-grayscale">
                        ...
                        <div className="price-value">
                          {SKIN_DATA[currentSkin].points}
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="profile-skin-button">
                    <button
                      style={{ backgroundColor: "#5a60ff4d" }}
                      className={`button px-4 py-3 rounded-xl ${
                        (currentSkin == skin ||
                          point < SKIN_DATA[currentSkin].points ||
                          level < SKIN_DATA[currentSkin].level) &&
                        `!bg-[#2B2717] text-opacity-50 text-[#FFFFFF]`
                      }`}
                      onClick={() => {
                        handleSkinPurchase(currentSkin);
                      }}
                      disabled={
                        currentSkin == skin ||
                        point < SKIN_DATA[currentSkin].points ||
                    ...
                    >
                      ...
                      level > SKIN_DATA[currentSkin].level ? (
                        ...
                        <span>
                          Reach the{" "}
                          {LEVEL_DATA[SKIN_DATA[currentSkin].level]?.name}{" "}
                          league to unlock the skin
                        </span>
                      )}
                    </button>
                  </div>
              ...
              <ul className="profile-skins-list">
                {Object.entries(SKIN_DATA).map((value, index) => (
                  <li
                    className={
                      currentSkin == value[0] ? `is-selected` : `is-locked`
                    }
                    key={`skin-list-${index}`}
                    onClick={() => {
                      setCurrentSkin(value[0]);
                    }}
                  >
                    <div
                      className={
                        skin == value[0] ? `is-current-skin` : `is-locked-skin`
                      }
                    >
                      <div className="icon">
                        {skin == value[0] ? (
